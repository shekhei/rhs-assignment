...
class RhsAssignmentGradleProjectImportHandler : GradleProjectImportHandler {
    ...
    val pluginJarName = "rhs-assignment-compiler-plugin"
...
//        get() = File(PathUtil.kotlinPathsForIdeaPlugin.libPath, pluginJpsJarName).absolutePath
    ...
    val logger = Logger.getInstance(RhsAssignmentGradleProjectImportHandler::class.java)

    fun modifyCompilerArguments(facet: KotlinFacet, buildSystemPluginJar: String) {
//        logger.error("We have it here!!!!!, modify CompilerArguments!!")
        ...
        val newPluginClasspaths = if (pluginWasEnabled) oldPluginClasspaths else oldPluginClasspaths
        commonArguments.pluginClasspaths = newPluginClasspaths.toTypedArray()
        facetSettings.compilerArguments = commonArguments
    }

    override fun importBySourceSet(facet: KotlinFacet, sourceSetNode: DataNode<GradleSourceSetData>) {
        modifyCompilerArguments(facet, pluginJarName)
    }
...
class RhsAssignmentResolverExtension: AbstractProjectResolverExtension() {
